...
export default function Contacts({data,username,userid}) {
  const socket=io.connect('https://mern-apiroyo3.onrender.com')
  ...
  const[msg,setmsg]=useState('')//message input text
   ...
   async function viewchat(e){
    console.log(e)
     ...
     console.log('te')
      ...
      const res=await axios.post('https://mern-apiroyo3.onrender.com/api/auth/getmsg',{from:userid,to:e})
   ...
   async function send(){
    const res=await axios.post('https://mern-apiroyo3.onrender.com/api/auth/sendmsg',{message:msg,from:userid,to:recid})
